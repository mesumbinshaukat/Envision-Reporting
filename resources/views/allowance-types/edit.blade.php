<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-navy-900" style="padding-right: 15px !important;">Edit Allowance Type</h2>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('allowance-types.update', $allowanceType) }}" class="bg-white border border-navy-900 rounded-lg p-6 space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-semibold text-navy-900 mb-1">Name (Machine Name) *</label>
                <input type="text" name="name" id="name" value="{{ old('name', $allowanceType->name) }}" required 
            ...
            <div>
                ...
                <input type="text" name="label" id="label" value="{{ old('label', $allowanceType->label) }}" required 
                    class="w-full px-4 py-2 border border-navy-900 rounded"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" 
                    class="w-full px-4 py-2 border border-navy-900 rounded">{{ old('description', $allowanceType->description) }}</textarea>
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $allowanceType->is_active) ? 'checked' : '' }} 
                    class="w-4 h-4 border-navy-900 rounded">
                <label for="is_active" class="ml-2 text-sm font-semibold text-navy-900">Active</label>
            </div>

            <div class="flex gap-4">
                <button type="submit" class="px-6 py-2 bg-navy-900 text-white rounded hover:bg-opacity-90">Update Allowance Type</button>
