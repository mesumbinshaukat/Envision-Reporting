<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-navy-900" style="padding-right: 15px !important;">Add Bonus</h2>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('bonuses.store') }}" class="bg-white border border-navy-900 rounded-lg p-6 space-y-4">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-4 py-2 border border-navy-900 rounded">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="release_type" id="release_type" required class="w-full px-4 py-2 border border-navy-900 rounded">
                    <option value="with_salary" {{ old('release_type') == 'with_salary' ? 'selected' : '' }}>With Salary</option>
                    <option value="separate" {{ old('release_type') == 'separate' ? 'selected' : '' }}>Separate (Immediate)</option>
                </select>
            </div>

            <div class="flex gap-4">
                <button type="submit" class="px-6 py-2 bg-navy-900 text-white rounded hover:bg-opacity-90">Add Bonus</button>
