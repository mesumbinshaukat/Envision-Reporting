<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-navy-900" style="padding-right: 15px !important;">Add User</h2>
    </x-slot>

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.users.store') }}" class="bg-white border border-navy-900 rounded-lg p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-600 mt-1">Admins are created only via the initial setup. Use moderators/supervisors for delegated access.</p>
                </div>

                <div>
                    <label for="password" class="block text-sm font-semibold text-navy-900 mb-1">Password *</label>
                    <input type="password" name="password" id="password" required minlength="8" class="w-full px-4 py-2 border border-navy-900 rounded">
                    <p class="text-xs text-gray-600 mt-1">Minimum 8 characters.</p>
                </div>
            </div>

            <div>
                <label for="password_confirmation" class="block text-sm font-semibold text-navy-900 mb-1">Confirm Password *</label>
                <input type="password" name="password_confirmation" id="password_confirmation" required minlength="8" class="w-full px-4 py-2 border border-navy-900 rounded">
            </div>
            ...
            <div class="border-t border-gray-300 pt-4 space-y-3">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-600">Choose exactly what this user can access. “Write” implies “Read”.</p>
                    </div>
                </div>
                        ...
                        <div class="border border-navy-200 rounded-lg p-4">
                            ...
                            <div class="space-y-2" data-group="{{ \Illuminate\Support\Str::slug($group) }}">
                                @foreach($features as $meta)
                                    @php
                                        $key = $meta['key'];
                                        $oldRead = old("permissions.$key.read");
                                        $oldWrite = old("permissions.$key.write");
                                        $readChecked = $oldRead ? true : false;
                                        $writeChecked = $oldWrite ? true : false;
                                    @endphp
            ...
            <div class="flex gap-4">
                <button type="submit" class="px-6 py-2 bg-navy-900 text-white rounded hover:bg-opacity-90">Create User</button>
