<x-app-layout>
    ...
    <div class="space-y-6" x-data="activityLogFilters()">
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm">
            <div class="text-blue-900">
                <strong>Retention Policy:</strong> Only the last {{ $retentionDays }} days of activity logs are retained automatically for each admin.
            </div>
            <form method="POST" action="{{ route('admin.activity-logs.cleanup') }}" onsubmit="return confirmActivityLogCleanup(event)">
                @csrf
    ...
    @push('scripts')
        <script>
            function confirmActivityLogCleanup(event) {
                const proceed = confirm('This will permanently remove all activity logs for your admin account. This action cannot be undone. Continue?');
                if (!proceed) {
