<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-navy-900" style="padding-right: 15px !important;">Assign Allowance to Employee</h2>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('employee-allowances.store') }}" class="bg-white border border-navy-900 rounded-lg p-6 space-y-4">
            @csrf
            ...
            <div>
                ...
                <select name="allowance_type_id" id="allowance_type_id" required class="w-full px-4 py-2 border border-navy-900 rounded">
                    ...
                    @foreach($allowanceTypes as $type)
                        <option value="{{ $type->id }}" {{ old('allowance_type_id') == $type->id ? 'selected' : '' }}>{{ $type->label }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex gap-4">
                <button type="submit" class="px-6 py-2 bg-navy-900 text-white rounded hover:bg-opacity-90">Assign Allowance</button>
