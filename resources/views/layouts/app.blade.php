        ...
        <div id="appShell" class="min-h-screen bg-gray-50 lg:pl-64 transition-[padding] duration-300">
            ...
            <aside id="sidebar" aria-label="Sidebar navigation" class="fixed inset-y-0 left-0 z-30 flex w-64 flex-col bg-white border-r border-navy-900 transition-transform duration-300 -translate-x-full lg:translate-x-0">
                ...
                <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                    @php
                        $navWebUser = auth()->guard('web')->check() ? auth()->guard('web')->user() : null;
                        $navWebUser?->loadMissing('featurePermissions');
                        $navIsEmployee = auth()->guard('employee')->check();
                    @endphp
                    @if($navIsEmployee || ($navWebUser && $navWebUser->canReadFeature('dashboard')))
                    ...
                    @endif
                    @if($navIsEmployee || ($navWebUser && $navWebUser->canReadFeature('clients')))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('employees'))
                    ...
                    @if($navIsEmployee || ($navWebUser && $navWebUser->canReadFeature('invoices')))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('expenses'))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('allowances'))
                        <a href="{{ route('employee-allowances.index') }}" class="flex items-center px-4 py-2 rounded {{ request()->routeIs('employee-allowances.*') || request()->routeIs('allowance-types.*') ? 'bg-navy-900 text-white' : 'text-navy-900 hover:bg-navy-900 hover:text-white' }}" title="Allowances">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path>
                            </svg>
                            <span class="ml-3 sidebar-text">Allowances</span>
                        </a>
                    @endif

                    @if($navWebUser && $navWebUser->canReadFeature('bonuses'))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('salary_releases'))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('reports'))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('attendance_admin'))
                        <!-- Attendance Dropdown -->
                        <div class="relative">
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('currencies'))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('activity_logs'))
                    ...
                    @if($navWebUser && $navWebUser->canReadFeature('users'))
                        <a href="{{ route('admin.users.index') }}" class="flex items-center px-4 py-2 rounded {{ request()->routeIs('admin.users.*') ? 'bg-navy-900 text-white' : 'text-navy-900 hover:bg-navy-900 hover:text-white' }}" title="Users">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4a4 4 0 100 8 4 4 0 000-8zm0 10c-4.418 0-8 2.239-8 5v1h16v-1c0-2.761-3.582-5-8-5z"></path>
                            </svg>
